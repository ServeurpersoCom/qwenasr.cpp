...
static bool is_important_sm(const char *name) {
  return (strstr(name, "attn_v.weight") != nullptr) ||
...
static bool is_important_l(const char *name) {
  return is_important_sm(name) ||
         (strstr(name, "attn_output.weight") != nullptr);
}
...
static bool should_quantize(int n_dims) { return n_dims >= 2; }
...
static enum ggml_type pick_type(const char *name, int n_dims,
  ...
  bool important =
      (v.bump_mode == 3) ? is_important_l(name) : is_important_sm(name);
...
static bool to_f32(const void *src, float *dst, int64_t n,
                   enum ggml_type type) {
  switch (type) {
  case GGML_TYPE_BF16:
...
int main(int argc, char **argv) {
  ...
  HANDLE fh = CreateFileW(winp_path.c_str(), GENERIC_READ, FILE_SHARE_READ,
  ...
  for (int i = 0; i < n_tensors; i++) {
    ...
    bool can_convert = (t->type == GGML_TYPE_BF16 || t->type == GGML_TYPE_F16 ||
  ...
  for (int i = 0; i < n_tensors; i++) {
    ...
    if (plan.quantize) {
      ...
      const size_t qsize =
          ggml_row_size(plan.target, n_per_row) * (size_t)nrows;

      std::vector<uint8_t> qbuf(qsize);
      ggml_quantize_chunk(plan.target, f32.data(), qbuf.data(), 0, nrows,
                          n_per_row, nullptr);
