...
static void asr_json_error(httplib::Response & res, int status, const char * type, const char * message) {
...
static void asr_handle_transcriptions(const server_config &    cfg,
    ...
    if (!req.form.has_file("file")) {
        asr_json_error(res, 400, "invalid_request_error", "missing 'file' field with the audio to transcribe");
        return;
    }
    ...
    int     n_samples = 0;
    float * pcm =
        audio_read_mono_buf((const uint8_t *) file.content.data(), file.content.size(), kModelSampleRate, &n_samples);
    if (!pcm || n_samples <= 0) {
        free(pcm);
        asr_json_error(res, 400, "invalid_request_error", "cannot decode audio, expects a WAV upload");
        return;
    }
    ...
    qa_status rc;
    {
        ...
        rc = qa_transcribe(g_ctx, pcm, (size_t) n_samples, kModelSampleRate, &tp, &text);
    }
    free(pcm);

    if (rc != QA_STATUS_OK) {
        asr_json_error(res, asr_status_to_http(rc), "server_error", qa_last_error());
        return;
    }
...
static void asr_handle_models(const httplib::Request &, httplib::Response & res) {
    yyjson_mut_doc * doc  = yyjson_mut_doc_new(NULL);
    yyjson_mut_val * root = yyjson_mut_obj(doc);
    yyjson_mut_doc_set_root(doc, root);
...
static void asr_handle_health(const httplib::Request &, httplib::Response & res) {
...
static int asr_server_run(const server_config & cfg) {
    ...
    svr.Post("/v1/audio/transcriptions", [&cfg](const httplib::Request & req, httplib::Response & res) {
