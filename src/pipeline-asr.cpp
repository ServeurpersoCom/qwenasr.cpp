...
static void dump_f32(const std::string & dir, const char * name, const std::vector<float> & v) {
...
static std::vector<float> run_mel(pipeline_asr * p, const std::vector<float> & samples, size_t * n_frames_out) {
    ...
    const size_t mem =
        ggml_tensor_overhead() * GGML_DEFAULT_GRAPH_SIZE + ggml_graph_overhead_custom(GGML_DEFAULT_GRAPH_SIZE, false);
    ...
    struct ggml_tensor * hann_in   = ggml_new_tensor_1d(gctx, GGML_TYPE_F32, cfg.n_fft);
    struct ggml_tensor * dft_re_in = ggml_new_tensor_2d(gctx, GGML_TYPE_F32, cfg.n_fft, c.n_freq);
    struct ggml_tensor * dft_im_in = ggml_new_tensor_2d(gctx, GGML_TYPE_F32, cfg.n_fft, c.n_freq);
    struct ggml_tensor * mel_b_in  = ggml_new_tensor_2d(gctx, GGML_TYPE_F32, c.n_freq, cfg.n_mels);
    ...
    struct ggml_tensor * mel = audio_mel_build_graph(gctx, audio_in, hann_in, dft_re_in, dft_im_in, mel_b_in, cfg);
    ggml_set_output(mel);

    struct ggml_cgraph * graph = ggml_new_graph_custom(gctx, GGML_DEFAULT_GRAPH_SIZE, false);
    ...
    ggml_backend_tensor_set(hann_in, c.hann.data(), 0, c.hann.size() * sizeof(float));
    ggml_backend_tensor_set(dft_re_in, c.dft_real.data(), 0, c.dft_real.size() * sizeof(float));
    ggml_backend_tensor_set(dft_im_in, c.dft_imag.data(), 0, c.dft_imag.size() * sizeof(float));
    ggml_backend_tensor_set(mel_b_in, c.mel_basis.data(), 0, c.mel_basis.size() * sizeof(float));
    if (ggml_backend_sched_graph_compute(p->sched, graph) != GGML_STATUS_SUCCESS) {
    ...
    ggml_backend_tensor_get(mel, log10_mel.data(), 0, log10_mel.size() * sizeof(float));

    std::vector<float> out;
    *n_frames_out = audio_mel_normalize(log10_mel, cfg.n_mels, n_frames_full, out);
    ggml_free(gctx);
    if (!p->dump_dir.empty()) {
...
static std::vector<float> run_tower(pipeline_asr *             p,
                                    int                        n_mels,
                                    const std::vector<float> & mel,
                                    size_t                     n_frames,
                                    int *                      n_states_out) {
    const AudioTowerConfig & tcfg            = p->tower_cfg;
    std::vector<int>         chunk_lengths   = audio_tower_chunk_lengths((int) n_frames, tcfg);
    const int                S               = audio_tower_seq_len(chunk_lengths);
    const int                window_aftercnn = audio_tower_conv_out(tcfg.chunk_mel) * tcfg.window_chunks;
    ...
    struct ggml_tensor * mel_in  = ggml_new_tensor_4d(gctx, GGML_TYPE_F32, (int64_t) n_frames, n_mels, 1, 1);
    struct ggml_tensor * pe_in   = ggml_new_tensor_2d(gctx, GGML_TYPE_F32, d_model, pe_rows);
    ...
    struct ggml_tensor * out = audio_tower_build(gctx, p->stem, p->enc, mel_in, pe_in, mask_in, chunk_lengths, &stem_t);
    ggml_set_output(out);
    if (dump) {
    ...
    if (dump) {
        ...
        ggml_backend_tensor_get(stem_t, stem_host.data(), 0, stem_host.size() * sizeof(float));
...
static std::vector<float> embed_tokens(pipeline_asr * p, const std::vector<int> & ids) {
...
pipeline_asr * pipeline_asr_load(const pipeline_asr_params & params) {
    ...
    if (!gf_load(&gf, params.model_path.c_str())) {
        qa_throw("pipeline_asr_load: cannot load gguf %s", params.model_path.c_str());
    }
    ...
    if (!graph_arena_init(&p->thinker_prefill_arena, thinker_graph_max_nodes(p->tw.num_hidden_layers)) ||
        !graph_arena_init(&p->thinker_decode_arena, thinker_graph_max_nodes(p->tw.num_hidden_layers))) {
...
static void asr_log_perf(const AsrPerf & p) {
    ...
    const double tok_s  = p.decode_ms > 0.0 ? (double) p.n_tokens / (p.decode_ms / 1000.0) : 0.0;
    ...
    qa_log(QA_LOG_INFO, "[Perf] Build %.1f ms (embed + audio splice)", p.build_ms);
    qa_log(QA_LOG_INFO, "[Perf] Prefill %.1f ms", p.prefill_ms);
    qa_log(QA_LOG_INFO, "[Perf] Decode %.1f ms (%d tokens, %.2f ms/tok, %.1f tok/s)", p.decode_ms, p.n_tokens, ms_tok,
           tok_s);
    qa_log(QA_LOG_INFO, "[Perf] Total %.1f ms (audio %.2f s, RTF %.3f)", p.total_ms, p.audio_sec, rtf);
}

qa_status pipeline_asr_run(pipeline_asr *               p,
                           const float *                pcm,
                           size_t                       n_samples,
                           int                          sample_rate,
                           const qa_transcribe_params * params,
                           std::string &                text) {
    Timer   t_total;
    AsrPerf perf   = {};
    ...
    if (sample_rate != kModelSampleRate) {
        int     n_out = 0;
        float * r     = audio_resample(pcm, (int) n_samples, sample_rate, kModelSampleRate, 1, &n_out);
    ...
    Timer              t_tower;
    int                S      = 0;
    std::vector<float> states = run_tower(p, p->mel_cfg.n_mels, mel, n_frames, &S);
    perf.tower_ms             = t_tower.ms();

    Timer             t_build;
    const std::string language = params->language ? resolve_language(params->language) : std::string();
    ...
    memcpy(embed.data() + (size_t) prompt.audio_offset * (size_t) hidden, states.data(),
           (size_t) S * (size_t) hidden * sizeof(float));
    perf.build_ms = t_build.ms();

    const int max_new = params->max_new_tokens > 0 ? params->max_new_tokens : QA_DEFAULT_MAX_NEW_TOKENS;

    KVCache kv;
    if (!kv_cache_init(&kv, p->tw.num_hidden_layers, p->tw.num_key_value_heads, p->tw.head_dim, T + max_new,
                       p->backend.backend)) {
    ...
    if (!thinker_forward_prefill(&p->tw, &kv, p->sched, &p->thinker_prefill_arena, embed.data(), T, p->flash_attn,
                                 p->clamp_fp16, !p->dump_dir.empty(), &out)) {
        kv_cache_free(&kv);
