...
static const int LANG_NAME_TO_ID_TABLE_N =
    (int)(sizeof(LANG_NAME_TO_ID_TABLE) / sizeof(LANG_NAME_TO_ID_TABLE[0]));
...
static std::string resolve_language(const std::string &language) {
  ...
  if (name_to_id.empty()) {
    for (int i = 0; i < LANG_NAME_TO_ID_TABLE_N; i++) {
      name_to_id[LANG_NAME_TO_ID_TABLE[i].first] =
          LANG_NAME_TO_ID_TABLE[i].second;
