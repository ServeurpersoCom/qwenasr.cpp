...
static inline void apply_repetition_penalty(float *logits, int V,
                                            const int32_t *history,
                                            int n_history, float penalty) {
...
static int sample_top_k_p(float *logits, int V, float temperature, int top_k,
                          float top_p, float rep_pen, const int32_t *history,
                          int n_history, int64_t seed, int64_t philox_subseq,
                          float *dump_u_out) {
  if (temperature <= 0.0f) {
    if (dump_u_out) {
      *dump_u_out = -1.0f;
  ...
  if (top_k > 0 && top_k < V) {
    ...
    std::nth_element(tmp_buf.begin(), tmp_buf.begin() + (top_k - 1),
                     tmp_buf.end(), std::greater<float>());
