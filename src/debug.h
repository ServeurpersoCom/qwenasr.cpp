...
static void debug_dump_3d(const DebugDumper *d, const char *name,
                          const float *data, int d0, int d1, int d2) {
  int shape[3] = {d0, d1, d2};
  debug_dump(d, name, data, shape, 3);
}

// Convenience: dump 4D tensor [d0, d1, d2, d3].
static void debug_dump_4d(const DebugDumper *d, const char *name,
                          const float *data, int d0, int d1, int d2, int d3) {
...
// loader side.
static void debug_dump_i32_as_f32(const DebugDumper *d, const char *name,
                                  const int32_t *data, const int *shape,
                                  int ndims) {
  if (!d->enabled) {
    return;
  }
