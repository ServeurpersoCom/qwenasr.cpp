...
static enum ggml_type gf_get_type(const GGUFModel & gf, const std::string & name) {
    struct ggml_tensor * src = ggml_get_tensor(gf.meta, name.c_str());
    if (!src) {
        qa_throw("[GGUF] tensor '%s' not in meta context (gf_get_type)", name.c_str());
    }
    return src->type;
}
...
static void gf_load_conv_f16(struct ggml_tensor * dst, const GGUFModel & gf, const std::string & name) {
    struct ggml_tensor * src = ggml_get_tensor(gf.meta, name.c_str());
    if (!src) {
        qa_throw("[GGUF] tensor '%s' not in meta context (conv f16 load)", name.c_str());
    }
    ...
    if (src->type == GGML_TYPE_F32) {
        ...
        if (!tr || !tr->to_float) {
            qa_throw("[GGUF] unsupported conv weight type %s for '%s'", ggml_type_name(src->type), name.c_str());
        }
...
static struct ggml_tensor * gf_load_qkv_fused(WeightCtx *         wctx,
    ...
    if (!q_src || !k_src || !v_src) {
        qa_throw("[GGUF] QKV tensor not found: %s / %s / %s", q_name.c_str(), k_name.c_str(), v_name.c_str());
    }
