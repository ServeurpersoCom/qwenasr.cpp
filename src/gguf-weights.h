...
//
// GGUF weight loader for all model components (LM, DiT, CondEncoder,
// TextEncoder, Detokenizer, VAE). All components use GGUF bf16 files generated
// by convert.py.
//
// Usage:
//   GGUFModel gf;
...
static bool gf_load(GGUFModel *gf, const char *path) {
  *gf = {};
  ...
  gf->fh = CreateFileW(wpath.c_str(), GENERIC_READ, FILE_SHARE_READ, NULL,
  ...
  gf->mapping =
      (uint8_t *)mmap(NULL, gf->file_size, PROT_READ, MAP_PRIVATE, gf->fd, 0);
...
gf_load_tensor(WeightCtx *wctx, const GGUFModel &gf, const std::string &name,
  ...
  struct ggml_tensor *tensor =
      ggml_new_tensor(wctx->ctx, src->type, n_dims, ne);
...
static struct ggml_tensor *gf_try_load_tensor(WeightCtx *wctx,
                                              const GGUFModel &gf,
...
// Best for small tensors: norms [H], QK-norms [D], scale_shift_table [H,6],
// biases.
static struct ggml_tensor *gf_load_tensor_f32(WeightCtx *wctx,
                                              const GGUFModel &gf,
                                              const std::string &name) {
  int64_t idx = gguf_find_tensor(gf.gguf, name.c_str());
  if (idx < 0) {
    qa_throw("[GGUF] tensor '%s' not found (f32 load)", name.c_str());
  }
  struct ggml_tensor *src = ggml_get_tensor(gf.meta, name.c_str());
  ...
  if (src->type != GGML_TYPE_BF16 && src->type != GGML_TYPE_F16) {
    fprintf(stderr,
            "[GGUF] WARNING: gf_load_tensor_f32 unsupported type %d for '%s', "
            "loading as-is\n",
            src->type, name.c_str());
    return gf_load_tensor(wctx, gf, name);
  }
  ...
  // Create F32 tensor
  struct ggml_tensor *tensor =
      ggml_new_tensor(wctx->ctx, GGML_TYPE_F32, n_dims, ne);
  ggml_set_name(tensor, name.c_str());
...
static enum ggml_type gf_get_type(const GGUFModel &gf,
                                  const std::string &name) {
  struct ggml_tensor *src = ggml_get_tensor(gf.meta, name.c_str());
  if (!src) {
    qa_throw("[GGUF] tensor '%s' not in meta context (gf_get_type)",
             name.c_str());
  }
  return src->type;
}
...
static void gf_load_conv_f16(struct ggml_tensor *dst, const GGUFModel &gf,
                             const std::string &name) {
  struct ggml_tensor *src = ggml_get_tensor(gf.meta, name.c_str());
  if (!src) {
    qa_throw("[GGUF] tensor '%s' not in meta context (conv f16 load)",
             name.c_str());
  }
  ...
  if (src->type == GGML_TYPE_F32) {
    ...
    if (!tr || !tr->to_float) {
      qa_throw("[GGUF] unsupported conv weight type %s for '%s'",
               ggml_type_name(src->type), name.c_str());
    }
...
// Fuse Q, K, V projection weights into a single tensor [ne0, q_ne1 + k_ne1 +
// v_ne1]. Works for any quantized type since quantization is per-row (along
// ne[0]). The fused tensor data is q rows || k rows || v rows (contiguous).
static struct ggml_tensor *gf_load_qkv_fused(WeightCtx *wctx,
  ...
  if (!q_src || !k_src || !v_src) {
    qa_throw("[GGUF] QKV tensor not found: %s / %s / %s", q_name.c_str(),
             k_name.c_str(), v_name.c_str());
  }
  ...
  wctx->pending.push_back(
      {fused, get_data(v_name), v_bytes, q_bytes + k_bytes});
  return fused;
}
