...
qa_context * qa_init(const struct qa_init_params * params) {
    ...
    if (params->abi_version > QA_ABI_VERSION) {
        qa_set_error("qa_init: params->abi_version %d > QA_ABI_VERSION %d", params->abi_version, QA_ABI_VERSION);
        return nullptr;
    }
    try {
...
enum qa_status qa_transcribe(qa_context *                        ctx,
                             const float *                       samples,
                             size_t                              n_samples,
                             int                                 sample_rate,
    ...
    if (p->abi_version > QA_ABI_VERSION) {
        qa_set_error("qa_transcribe: params->abi_version %d too new", p->abi_version);
        return QA_STATUS_INVALID_PARAMS;
    }

    std::string     text;
    const qa_status rc = pipeline_asr_run(ctx->pipe, samples, n_samples, sample_rate, p, text);
    if (rc != QA_STATUS_OK) {
        return rc;
    }
