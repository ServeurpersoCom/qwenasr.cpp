...
static void audio_enc_compute_pe(int channels, int length, std::vector<float> & out) {
    ...
    for (int t = 0; t < length; t++) {
        for (int i = 0; i < half; i++) {
            ...
            out[(size_t) t * (size_t) channels + (size_t) (half + i)] = (float) std::cos(s);
        }
    }
}
...
// harness).
static bool audio_enc_load(AudioEnc * w, const GGUFModel & gf, ggml_backend_t backend) {
    ...
    fprintf(stderr,
            ...
            "FFN %d, output_dim %d\n",
            w->cfg.n_layer, w->cfg.d_model, w->cfg.n_head, w->cfg.d_model / w->cfg.n_head, w->cfg.ffn,
            w->cfg.output_dim);
    return ok;
}
...
static struct ggml_tensor * audio_enc_build(struct ggml_context * ctx,
    ...
    for (int l = 0; l < cfg.n_layer; l++) {
        ...
        q = ggml_permute(ctx, ggml_reshape_3d(ctx, q, head_dim, cfg.n_head, T), 0, 2, 1, 3);
        k = ggml_permute(ctx, ggml_reshape_3d(ctx, k, head_dim, cfg.n_head, T), 0, 2, 1, 3);

        struct ggml_tensor * kq = ggml_mul_mat(ctx, k, q);  // [T, T, n_head]
        kq                      = ggml_soft_max_ext(ctx, kq, mask, scale, 0.0f);

        struct ggml_tensor * vt =
            ggml_cont(ctx, ggml_permute(ctx, ggml_reshape_3d(ctx, v, head_dim, cfg.n_head, T), 1, 2, 0, 3));
        struct ggml_tensor * kqv = ggml_mul_mat(ctx, vt, kq);           // [head_dim, T, n_head]
