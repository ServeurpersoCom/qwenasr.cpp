...
static bool is_letter(int cp) {
  ...
  // Latin Extended: U+00C0-U+00D6, U+00D8-U+00F6, U+00F8-U+01BF + Latin
  // Extended-A/B
...
static bool is_whitespace(int cp) {
  return cp == ' ' || cp == '\t' || cp == '\n' || cp == '\r' || cp == 0x0B ||
         cp == 0x0C || cp == 0xA0 || cp == 0x2000 || cp == 0x2001 ||
         cp == 0x2002 || cp == 0x200B;
}
...
static std::vector<std::string> gpt2_pre_tokenize(const std::string &text) {
  ...
  while (i < len) {
    ...
    if (!is_newline(cp) && !is_letter(cp) && !is_digit(cp) &&
        !is_whitespace(cp)) {
    ...
    // This peels off leading spaces, leaving the last space to combine with the
    // next word
    if (is_whitespace(cp)) {
      int start = i;
      // Find end of whitespace run
      int ws_end = i + adv;
      while (ws_end < len && is_whitespace((unsigned char)s[ws_end]) &&
             !is_newline((unsigned char)s[ws_end])) {
        ws_end++;
      }
      // Check what follows the whitespace run
      bool followed_by_non_ws =
          (ws_end < len && !is_whitespace((unsigned char)s[ws_end]) &&
           !is_newline((unsigned char)s[ws_end]));
    ...
    // Rule 4: [^\s\p{L}\p{N}]+[\r\n]*  (punctuation/symbols)
    {
      ...
      while (i < len) {
        int a2;
        int cp2 = utf8_codepoint(s + i, &a2);
        if (is_whitespace(cp2) || is_letter(cp2) || is_digit(cp2) ||
            is_newline(cp2)) {
          break;
        }
        i += a2;
...
static std::string byte_level_encode(const BPETokenizer *tok,
...
bpe_merge(const std::unordered_map<std::string, int> &merge_rank,
...
static void encode_chunk(const BPETokenizer *tok, const std::string &chunk,
  ...
  for (const auto &piece : merged) {
    ...
    if (it != tok->vocab.end()) {
      ...
      // Fallback: encode each byte individually (should not happen with
      // byte-level BPE)
...
static std::vector<int> bpe_encode(const BPETokenizer *tok,
                                   const std::string &text,
                                   bool add_eos = true) {
  std::vector<int> ids;
