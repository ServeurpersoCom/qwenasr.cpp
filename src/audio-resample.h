...
// Build the Hann-sinc polyphase kernel [new_freq_red, kernel_size] in row
// major. new_freq_red and orig_freq_red are sr_out/gcd and sr_in/gcd
// respectively.
static std::vector<float> audio_resample_build_kernel(int orig, int newf,
                                                      int *out_width,
                                                      int *out_kernel_size) {
...
// Resample one mono channel from sr_in to sr_out. Returns
// ceil(sr_out*n_in/sr_in) samples. Caller passes the kernel + width built once
// via audio_resample_build_kernel.
static void audio_resample_apply_mono(const float *in, int n_in, int orig,
                                      int newf, int width, int kernel_size,
...
static float *audio_resample(const float *in, int n_in, int sr_in, int sr_out,
                             int nch, int *n_out) {
  ...
  std::vector<float> kernel =
      audio_resample_build_kernel(orig, newf, &width, &kernel_size);

  long long target =
      (long long)std::ceil((double)sr_out * (double)n_in / (double)sr_in);
  if (target <= 0) {
    *n_out = 0;
    return NULL;
  }
  ...
  *n_out = (int)target;
  ...
  for (int ch = 0; ch < nch; ch++) {
    ...
    audio_resample_apply_mono(src, n_in, orig, newf, width, kernel_size,
                              kernel.data(), dst, target);
  }
