#pragma once
// audio-io.h: WAV read/write for qwenasr.cpp.
...
// flat mono float [T] for writes (qwenasr is mono only).
...
static float *audio_io_read_wav_buf(const uint8_t *data, size_t size,
                                    int *T_out, int *sr_out) {
  *T_out = 0;
  *sr_out = 0;
...
static float wav_sanitize(float x) { return std::isfinite(x) ? x : 0.0f; }

// Classic RIFF header: fmt_tag 1 (PCM int) or 3 (IEEE float), 16-byte fmt chunk
static void wav_write_header_basic(char *&p, int T_audio, int sr,
                                   int n_channels, int bits, uint16_t fmt_tag) {
  ...
  uint32_t data_size =
      (uint32_t)T_audio * (uint32_t)n_channels * bytes_per_sample;
...
static std::string audio_encode_wav_s16(const float *audio, int T_audio,
                                        int sr) {
  int n_channels = 1;
...
// Clamps to [-1, +1], coerces NaN/Inf to zero.
static std::string audio_encode_wav_s24(const float *audio, int T_audio,
                                        int sr) {
  int n_channels = 1;
...
static std::string audio_encode_wav_f32(const float *audio, int T_audio,
                                        int sr) {
  int n_channels = 1;
...
static bool audio_write_wav(const char *path, const float *audio, int T_audio,
                            int sr, WavFormat fmt = WAV_S16) {
  ...
  const char *fmt_name = (fmt == WAV_S16)   ? "S16"
                         : (fmt == WAV_S24) ? "S24"
                                            : "F32";
...
static bool wav_stream_write_header(wav_stream *ws) {
  ...
  uint32_t bytes_per_sample = (uint32_t)bits / 8;
  uint32_t byte_rate =
      (uint32_t)ws->sr * (uint32_t)n_channels * bytes_per_sample;
  uint16_t block_align = (uint16_t)(n_channels * (int)bytes_per_sample);
...
static void wav_stream_close(wav_stream *ws) { fflush(ws->fp); }
