#pragma once
// graph-arena.h: persistent no_alloc ggml context reused across graph builds.
// Rebuilding an identical graph into the same arena lands every node at a
// stable address, so the backend CUDA graph cache (keyed on the first node
// pointer) resolves to the same executable instance at every decode step
...
static bool graph_arena_init(GraphArena *a, int max_nodes) {
  const size_t bytes = ggml_tensor_overhead() * (size_t)max_nodes +
