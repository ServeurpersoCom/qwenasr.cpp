...
static BackendPair backend_init(const char *label) {
  if (g_backend_refs > 0) {
    g_backend_refs++;
    qa_log(QA_LOG_INFO, "[Load] %s backend: %s (shared)", label,
           ggml_backend_name(g_backend_cache.backend));
  ...
  if (force_backend) {
    ...
    if (!bp.backend) {
      ...
      qa_log(QA_LOG_ERROR, "%s", msg.c_str());
      return BackendPair{};
    }
  } else {
  ...
  if (!bp.backend) {
    qa_log(QA_LOG_ERROR, "[Load] no backend available");
    return BackendPair{};
  }
  ...
  if (!bp.cpu_backend) {
    qa_log(QA_LOG_ERROR, "[Load] failed to init CPU backend");
  ...
  qa_log(QA_LOG_INFO, "[Load] %s backend: %s (CPU threads: %d)", label,
         ggml_backend_name(bp.backend), n_threads);
...
static void backend_release(ggml_backend_t backend,
                            ggml_backend_t cpu_backend) {
  if (g_backend_refs <= 0) {
    return;
  }
...
static ggml_backend_sched_t backend_sched_new(BackendPair bp, int max_nodes) {
  ...
  if (n == 2) {
    ...
    ggml_backend_buffer_type_t host_buft =
        gpu_dev ? ggml_backend_dev_host_buffer_type(gpu_dev) : NULL;
    bufts[1] = host_buft ? host_buft
                         : ggml_backend_get_default_buffer_type(bp.cpu_backend);
  }

  ggml_backend_sched_t sched =
      ggml_backend_sched_new(backends, bufts, n, max_nodes, false, true);
  if (!sched) {
    qa_log(QA_LOG_ERROR, "[Load] failed to create scheduler");
    return nullptr;
  }
  return sched;
}
