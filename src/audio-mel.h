...
struct AudioMelConstants {
  ...
  std::vector<float>
      mel_basis; // [n_mels, n_freq] row major  ne=(n_freq, n_mels)
};
...
static void audio_mel_compute_constants(const AudioMelConfig &cfg,
                                        AudioMelConstants &c) {
  c.cfg = cfg;
  ...
  for (int i = 0; i < cfg.n_fft; i++) {
    c.hann[(size_t)i] =
        0.5f *
        (1.0f - (float)std::cos(2.0 * M_PI * (double)i / (double)cfg.n_fft));
  }
  ...
  for (int k = 0; k < c.n_freq; k++) {
    for (int n = 0; n < cfg.n_fft; n++) {
      ...
      c.dft_real[(size_t)k * (size_t)cfg.n_fft + (size_t)n] =
          (float)std::cos(th);
      c.dft_imag[(size_t)k * (size_t)cfg.n_fft + (size_t)n] =
          (float)(-std::sin(th));
    }
  }

  const float fmax =
      (cfg.fmax <= 0.0f) ? (float)cfg.sample_rate * 0.5f : cfg.fmax;
...
static struct ggml_tensor *audio_mel_build_graph(
    struct ggml_context *ctx, struct ggml_tensor *audio_padded,
    struct ggml_tensor *hann, struct ggml_tensor *dft_real,
    struct ggml_tensor *dft_imag, struct ggml_tensor *mel_basis,
    const AudioMelConfig &cfg) {
  const int n_fft = cfg.n_fft;
  const int hop = cfg.hop;

  struct ggml_tensor *a4d =
      ggml_reshape_4d(ctx, audio_padded, audio_padded->ne[0], 1, 1, 1);
  struct ggml_tensor *dummy =
      ggml_new_tensor_4d(ctx, GGML_TYPE_F32, n_fft, 1, 1, 1);
  ggml_set_name(dummy, "mel.im2col_dummy_kernel");

  struct ggml_tensor *frames =
      ggml_im2col(ctx, dummy, a4d, hop, 1, 0, 0, 1, 1, false, GGML_TYPE_F32);
  frames = ggml_reshape_2d(ctx, frames, n_fft, frames->ne[1]);

  frames = ggml_mul(ctx, frames, ggml_reshape_2d(ctx, hann, n_fft, 1));

  struct ggml_tensor *spec_re =
      ggml_mul_mat(ctx, dft_real, frames); // [n_freq, n_frames]
  ...
  struct ggml_tensor *power =
      ggml_add(ctx, ggml_sqr(ctx, spec_re), ggml_sqr(ctx, spec_im));
...
static size_t audio_mel_normalize(const std::vector<float> &log10_mel,
                                  int n_mels, size_t n_frames_full,
