...
static bool thinker_weights_load(ThinkerWeights *tw, const GGUFModel &gf,
  ...
  tw->num_key_value_heads =
      (int)gf_get_u32(gf, "qwenasr.attention.head_count_kv");
  ...
  if (tw->num_hidden_layers <= 0 || tw->hidden_size <= 0) {
    fprintf(stderr,
            "[Thinker] FATAL: invalid hyperparameters (layers=%d hidden=%d)\n",
  ...
  tw->token_embd =
      gf_load_tensor_f32(&tw->wctx, gf, "thinker.token_embd.weight");
  tw->output_norm_w =
      gf_load_tensor_f32(&tw->wctx, gf, "thinker.output_norm.weight");
  ...
  for (int l = 0; l < tw->num_hidden_layers; l++) {
    ...
    layer.input_norm_w =
        gf_load_tensor_f32(&tw->wctx, gf, p + "attn_norm.weight");
    layer.post_attn_norm_w =
        gf_load_tensor_f32(&tw->wctx, gf, p + "ffn_norm.weight");
    layer.attn.q_proj_w =
        gf_load_tensor_f32(&tw->wctx, gf, p + "attn_q.weight");
    layer.attn.k_proj_w =
        gf_load_tensor_f32(&tw->wctx, gf, p + "attn_k.weight");
    layer.attn.v_proj_w =
        gf_load_tensor_f32(&tw->wctx, gf, p + "attn_v.weight");
    layer.attn.o_proj_w =
        gf_load_tensor_f32(&tw->wctx, gf, p + "attn_output.weight");
    layer.attn.q_norm_w =
        gf_load_tensor_f32(&tw->wctx, gf, p + "attn_q_norm.weight");
    layer.attn.k_norm_w =
        gf_load_tensor_f32(&tw->wctx, gf, p + "attn_k_norm.weight");
    layer.mlp.gate_proj_w =
        gf_load_tensor_f32(&tw->wctx, gf, p + "ffn_gate.weight");
    layer.mlp.up_proj_w =
        gf_load_tensor_f32(&tw->wctx, gf, p + "ffn_up.weight");
    layer.mlp.down_proj_w =
        gf_load_tensor_f32(&tw->wctx, gf, p + "ffn_down.weight");
  }
  ...
  fprintf(stderr,
          ...
          tw->num_hidden_layers, tw->hidden_size, tw->num_attention_heads,
          tw->num_key_value_heads, tw->head_dim, tw->intermediate_size,
          (double)tw->rope_theta, tw->vocab_size, (int)tw->tie_word_embeddings);
  return true;
}
